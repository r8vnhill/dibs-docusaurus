...
<details>
    ...
    <Tabs groupId={"os"}>
        <TabItem value="Windows" label="Windows">
            ```powershell
            New-Item -Path 'anonymous' -ItemType 'directory'
            '// Intentionally left blank' |
                Out-File -FilePath 'anonymous/build.gradle.kts'
            ```
        </TabItem>
        <TabItem value="Windows (corto)" label="Windows (corto)">
            ```powershell
            md anonymous
            '// Intentionally left blank' > anonymous/build.gradle.kts
            ```
        </TabItem>
        <TabItem value="Linux/Mac" label="Linux/Mac">
            ```bash
            mkdir anonymous
            echo '// Intentionally left blank' > anonymous/build.gradle.kts
            ```
        </TabItem>
    </Tabs>
...
<details>
    ...
    <Tabs groupId={"os"}>
        <TabItem value="Windows" label="Windows">
            ...
            ```powershell
            $AnonymousTestDir = "anonymous\src\test\kotlin\$Group\anonymous"
            New-Item -Path "$AnonymousTestDir\AnonymousObjectTest.kt" `
                -ItemType "file" -Force
            ```
        </TabItem>
        <TabItem value="Windows (corto)" label="Windows (corto)">
            ...
            ```powershell
            $AnonymousTestDir = "anonymous\src\test\kotlin\$Group\anonymous"
            ni "$AnonymousTestDir\AnonymousObjectTest.kt" -i 'f' -f
            ```
        </TabItem>
        <TabItem value="Linux/Mac" label="Linux/Mac">
            ```bash
            ...
            ```bash
            ANONYMOUS_TEST_DIR="anonymous/src/test/kotlin/$GROUP/anonymous"
            mkdir -p $ANONYMOUS_TEST_DIR
            touch $ANONYMOUS_TEST_DIR/AnonymousObjectTest.kt
            ```
        </TabItem>
    </Tabs>
</details>
...
<details>
    ...
    <Tabs groupId={"os"}>
        <TabItem value="Windows" label="Windows">
            ```powershell
            $ValidatorTestDir = "$AnonymousTestDir\validation"
            $ValidatorMainDir = "anonymous\src\main\kotlin\$Group\anonymous\validation"
            New-Item -Path "$ValidatorTestDir\ValidatorTest.kt" `
                -ItemType "file" -Force
            New-Item -Path "$ValidatorMainDir\Validator.kt" `
                -ItemType "file" -Force
            ```
        </TabItem>
        <TabItem value="Windows (corto)" label="Windows (corto)">
            ...
            $ValidatorTestDir = "$AnonymousTestDir\validation"
            $ValidatorMainDir = "anonymous\src\main\kotlin\$Group\anonymous\validation"
            ni "$ValidatorTestDir\ValidatorTest.kt" -i 'f' -f
            ni "$ValidatorMainDir\Validator.kt" -i 'f' -f
            ```
        </TabItem>
        <TabItem value="Linux/Mac" label="Linux/Mac">
            ```bash
            VALIDATOR_TEST_DIR="$AnonymousTestDir/validation"
            VALIDATOR_MAIN_DIR="anonymous/src/main/kotlin/$GROUP/anonymous/validation"
            mkdir -p $VALIDATOR_TEST_DIR
            mkdir -p $VALIDATOR_MAIN_DIR
            touch $VALIDATOR_TEST_DIR/ValidatorTest.kt
            touch $VALIDATOR_MAIN_DIR/Validator.kt
            ```
        </TabItem>
    </Tabs>
</details>
...
<details>
    ...
    <Tabs groupId={"os"}>
        <TabItem value="Windows" label="Windows">
            ```powershell
            $AnonymousMainDir = "anonymous\src\main\kotlin\$Group\anonymous"
            $PeopleTestDir = "$AnonymousTestDir\people"
            $PeopleMainDir = "$AnonymousMainDir\people"
            New-Item -Path "$PeopleTestDir\PersonTest.kt" `
                -ItemType "file" -Force
            New-Item -Path "$PeopleMainDir\Person.kt" `
                -ItemType "file" -Force
            New-Item -Path "$PeopleMainDir\AbstractPerson.kt" `
                -ItemType "file" -Force
            ```
        </TabItem>
        <TabItem value="Windows (corto)" label="Windows (corto)">
            ...
            $PeopleTestDir = "$AnonymousTestDir\people"
            $PeopleMainDir = "$AnonymousMainDir\people"
            ni "$PeopleTestDir\PersonTest.kt" -i 'f' -f
            ni "$PeopleMainDir\Person.kt" -i 'f' -f
            ni "$PeopleMainDir\AbstractPerson.kt" -i 'f' -f
            ```
        </TabItem>
        <TabItem value="Linux/Mac" label="Linux/Mac">
            ```bash
            ANONYMOUS_MAIN_DIR="anonymous/src/main/kotlin/$GROUP/anonymous"
            PEOPLE_TEST_DIR="$AnonymousTestDir/people"
            PEOPLE_MAIN_DIR="$ANONYMOUS_MAIN_DIR/people"
            mkdir -p $PEOPLE_TEST_DIR
            mkdir -p $PEOPLE_MAIN_DIR
            touch $PEOPLE_TEST_DIR/PersonTest.kt
            touch $PEOPLE_MAIN_DIR/Person.kt
            touch $PEOPLE_MAIN_DIR/AbstractPerson.kt
            ```
        </TabItem>
    </Tabs>
</details>
...
<ProCons>
    <Pros>
        - **Prototipos rápidos**: Puedes crear rápidamente instancias de clases abstractas sin necesidad de una implementación completa, lo que te permite enfocarte en aspectos específicos del código que estás probando.
        - **Aislamiento del comportamiento**: Al sobrescribir métodos abstractos de manera simplificada, puedes aislar la lógica que realmente te interesa probar, evitando la complejidad innecesaria.
        - **Flexibilidad**: Las **object expressions** ofrecen una forma flexible y ligera de modificar comportamientos, sin necesidad de generar nuevas subclases en archivos separados.        
    </Pros>
</ProCons>
