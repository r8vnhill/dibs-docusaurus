...
Desarrolla un conjunto de data classes que permitan gestionar libros (`Book`) y préstamos (`Loan`) en una biblioteca.

El sistema deberá incluir funcionalidades para agregar libros, realizar préstamos y devoluciones, y visualizar información de los libros.
</TabItem>
...
```kotlin
data class Loan(val book: Book, val startDate: String, val endDate: String)
```
...
class Library {
    ...
    val loans: List<Loan> get() = _loans

    fun addBook(book: Book) {
        _books += book
    }

    fun registerLoan(book: Book, startDate: String, endDate: String) {
        if (book in _books) {
            _loans += Loan(book, startDate, endDate)
        }
    }
}
```
