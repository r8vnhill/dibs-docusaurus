...
## Ejemplo: Creando un matcher como función de extensión

En la lección sobre [matchers personalizados](/docs/testing/matchers/custom) vimos cómo crear nuestros propios matchers para realizar comparaciones más legibles en las pruebas, esto nos permitía escribir expresiones como `2 should beEven()`, pero ¿qué pasa si queremos encadenar varios matchers como se hace con los matchers predefinidos de Kotest?

Primero definamos cómo esperamos que se vea la sintaxis de los matchers personalizados con un test:



## Extender clases genéricas: Añadiendo una función a `List<T>`
