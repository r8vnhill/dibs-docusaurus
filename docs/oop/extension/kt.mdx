...
println(numbers.second()) // Output: 20
```
...
println(transposed) // Output: [[1, 4], [2, 5], [3, 6]]
```
...
class Host(val hostname: String) {
    fun printConnectionString(port: Int) {
        fun Int.formatPort() = "Port: $this"
        println("$hostname connects to ${port.formatPort()}")
    }
}

val host = Host("localhost")
host.printConnectionString(8080)
// Output: localhost connects to Port: 8080
```
