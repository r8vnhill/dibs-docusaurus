...
<details>
    ...
    Es decir, `in` es simplemente azúcar sintáctica[^1] para `songs.contains("Perfect Day")`.
...
[^1]: Se llama **azúcar sintáctica** (*syntactic sugar*) a una forma más conveniente o legible de escribir algo que, en realidad, se traduce a una forma más básica del  lenguaje. En este caso, escribir `x in y` es azúcar sintáctica para `y.contains(x)`. Este proceso de traducir una construcción más compleja o amigable a una forma fundamental del lenguaje se conoce como **desugarización** (*desugaring*).
