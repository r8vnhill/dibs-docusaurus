...
- [🌐 Conditions and loops](https://kotlinlang.org/docs/control-flow.html) de la **documentación oficial de Kotlin**: Introducción al control de flujo en Kotlin mediante `if` y `when` como expresiones y declaraciones, con ejemplos de uso, exhaustividad, evaluación por ramas y comparaciones con otros lenguajes.

### 🔹 Adicionales

- 📚 "Conditionals" en *"Kotlin Programming: The Big Nerd Ranch Guide"* de **Andrew Bailey, David Greenhalgh y Josh Skeen**: Introducción al control de flujo en Kotlin mediante `if`, `else` y `when`, con ejemplos aplicados al proyecto *bounty-board* para definir misiones con dificultad escalable.
- 📚 "Kotlin basics" (pp. 17–43) en *"Kotlin in action"* de **Dmitry Jemerov y Svetlana Isakova**: Este capítulo presenta `if` y `when` como expresiones con valor de retorno, más poderosas y expresivas que sus equivalentes en Java, lo que permite escribir control de flujo más conciso e idiomático en Kotlin.
