...
Cuando diseñamos bibliotecas, muchas veces necesitamos representar **una entre varias alternativas posibles**: por ejemplo, el estado de una conexión, un nivel de log, o un resultado predecible como `OK` o `ERROR`. Estas situaciones corresponden a lo que en teoría de tipos se conoce como un **tipo suma**: una estructura que encapsula **una única alternativa válida** entre varias opciones posibles.

Kotlin nos ofrece una herramienta directa para modelar estos tipos: las **enumeraciones** (`enum class`). A diferencia de soluciones más frágiles como usar `String`, las enumeraciones permiten que el compilador garantice que:

- solo se usen valores válidos,
...
<details>
    ...
    Además, el símbolo `|` puede ser **más ambiguo** en algunos lenguajes como **Scala 3** o **Python**, donde también se usa para representar **tipos unión** entre clases o interfaces, no necesariamente disjuntos:

    ```scala
    def parse(input: String): Int | String = ???
    ```

    En estos contextos, `|` denota simplemente que el valor puede ser de **uno u otro tipo**, sin requerir que las alternativas estén en un conjunto cerrado como en un tipo suma clásico.
</details>
...
<Explanation>
    El uso de `when` con `enum class` nos permite escribir código **más seguro y explícito**, porque el compilador sabe exactamente cuántos casos hay que cubrir.  
    Si omitimos alguna alternativa, el compilador mostrará un **error o advertencia**, obligándonos a completar todos los casos definidos en la enumeración.  

    Esto es posible porque las `enum class` definen un **conjunto cerrado** de valores: no es posible que existan más casos fuera de los que ya están listados en la definición.  
    Gracias a esta propiedad, evitamos errores sutiles como olvidarse de manejar un caso nuevo al extender una enumeración.  

    Además, este enfoque mejora la **legibilidad**, porque cualquier persona que lea el código puede ver de forma explícita qué ocurre en cada situación posible, sin ambigüedades ni valores "mágicos".
</Explanation>
...
<details>
    ...
    Sin embargo, es importante tener en cuenta que esta característica es **experimental** y puede cambiar en futuras versiones de Kotlin.
...
Las enumeraciones (`enum class`) son una herramienta fundamental para representar tipos suma en Kotlin. Al modelar conjuntos finitos de alternativas conocidas, nos permiten escribir código **más seguro, expresivo y fácil de mantener**. Esta estructura es especialmente útil en bibliotecas que requieren representar **estados, modos o categorías cerradas**, donde cada valor tiene un significado claro y exclusivo.

El soporte del compilador para el **chequeo exhaustivo** en expresiones `when` sobre enumeraciones refuerza la corrección del código al alertar sobre casos no manejados. Y con la incorporación de **context-sensitive resolution** en versiones recientes de Kotlin, se mejora aún más la legibilidad del código al reducir el ruido sintáctico.
...
<ExcludeFromReadingTime>
    ...
    - 📕 "Item 41: Use Enum to Represent a List of Values." (pp. 249-255) en *Effective Kotlin: Best Practices, 2nd ed.* por **Marcin Moskała**: Explica cómo usar `enum class` para representar conjuntos fijos de valores, destacando sus ventajas en términos de serialización, iteración, ordenación y extensión con propiedades o funciones específicas por ítem. También compara `enum` con `sealed class`, señalando que las primeras son preferibles para representar valores constantes simples, mientras que las segundas son más adecuadas cuando se prevé agregar datos o lógica más compleja a cada caso.
