...
Esto se parece al enfoque de Kotlin, donde las variables son no anulables por defecto, y debes declarar explícitamente si un valor puede ser `null` utilizando `?`. Si intentas asignar un valor nulo a una variable no anulable en C#, obtendrás una advertencia en tiempo de compilación, similar a Kotlin.
...
```csharp showLineNumbers
int? length = nullableString?.Length;  // Si es nulo, no lanza excepción, sino que devuelve null
int? firstElement = nullableString?[0];  // Si es nulo o la cadena está vacía, devuelve null
```
...
```csharp showLineNumbers
int length = nullableString?.Length ?? -1;  // Si es nulo, devuelve -1
```
...
public static class Extensions
{
    public static TResult? Let<T, TResult>(
        this T? value,
        Func<T, TResult> func
    ) where T : class where TResult : class => value != null ? func(value) : null;
}

nullableString?.Let(s => Console.WriteLine(s.ToUpper()));
```
