...
<details>
    <summary>
        Llaves en `if`: ¿cuándo son necesarias?
    </summary>
    
    - Si el bloque `if` o `else` contiene **más de una línea**, **las llaves `{}` son obligatorias**.
    - Si contiene **una sola expresión**, puedes omitirlas, aunque **es recomendable incluirlas** para mejorar la legibilidad y evitar errores sutiles al agregar nuevas líneas más adelante.

    ```kotlin
    // Válido pero propenso a errores si se extiende en el futuro
    if (x > 0)
        println("Positive")
    else
        println("Negative")

    // Mejor (más claro y seguro)
    if (x > 0) {
        println("Positive")
    } else {
        println("Negative")
...
La expresión `when` en Kotlin es una alternativa más concisa y legible a múltiples estructuras `if-else if`. Además, **devuelve un valor**, lo que la convierte en una herramienta versátil para el control de flujo.
...
<Explanation>
    - **`when (status)`** evalúa el valor de `status` y compara contra distintos casos.
    - **`200, 201, 204 -> "Success"`** muestra cómo agrupar múltiples valores en una sola rama usando comas.
    - **`is Int`** y **`!is String`** usan comprobación de tipo (*type checking*) para manejar casos más generales.
    - **`else`** actúa como un caso por defecto, obligatorio si no se cubren todos los posibles valores.
    - Al ser una expresión, `when` **devuelve directamente un valor**, por lo que se puede usar en asignaciones o como resultado de una función.
</Explanation>
