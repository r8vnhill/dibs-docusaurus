...
<Explanation>
    ...
    A diferencia de lenguajes como Java o C#, Kotlin **no utiliza la palabra clave `new`** para crear objetos. Simplemente se llama al constructor como si fuera una función, lo que contribuye a una sintaxis más limpia y concisa.
...
## 🏗️ Constructores Primarios y Secundarios en Kotlin

Kotlin ofrece dos formas de inicializar clases: **constructores primarios**, que se definen junto al encabezado de la clase, y **constructores secundarios**, que permiten más flexibilidad cuando se requieren múltiples formas de construir un objeto.

### 🧱 Constructor Primario

El **constructor primario** se declara directamente en la cabecera de la clase y permite inicializar propiedades de forma concisa.

```kotlin showLineNumbers
class Person(val name: String, var age: Int)
```

- Ideal cuando deseas declarar e inicializar propiedades de manera directa.
- No permite incluir directamente lógica adicional, pero puedes usar un bloque `init` para ejecutar instrucciones extra al momento de construir la instancia.

#### 🔍 Uso de `init`

Si necesitas validar o ejecutar lógica durante la construcción, puedes usar un bloque `init`:
...
Este bloque se ejecuta inmediatamente después de que se evalúan los parámetros del constructor primario.

:::note `require` vs `check` vs `error`

Estas funciones permiten validar condiciones y lanzar excepciones de forma concisa, pero su uso recomendado depende del contexto:

- `require: (Boolean, () -> Any) -> Unit`: se usa para validar **argumentos** de una función. Lanza una `IllegalArgumentException` si la condición no se cumple.
- `check: (Boolean, () -> Any) -> Unit`: se usa para validar **el estado interno** del programa. Lanza una `IllegalStateException` si la condición no se cumple.
- `error: (Any) -> Unit`: lanza **incondicionalmente** una `IllegalStateException`. Es útil para marcar rutas de ejecución que no deberían ocurrir.

Usar la función adecuada según el contexto mejora la legibilidad del código y facilita el diagnóstico de errores.

:::

### 🛠️ Constructor Secundario

Un **constructor secundario** se define dentro del cuerpo de la clase y permite incluir lógica adicional. Se invoca al constructor primario mediante `this(...)`.
...
Los constructores secundarios son útiles cuando necesitas más de una forma de construir la instancia, por ejemplo:

- Al interoperar con Java (donde los parámetros por defecto no están disponibles).
- Cuando necesitas delegar a otro constructor con lógica adicional.
- En algunos patrones con herencia.

:::tip Parámetros por defecto

En la mayoría de los casos, los constructores secundarios no son necesarios gracias a los **parámetros por defecto** en Kotlin. Esta característica permite definir valores opcionales para los argumentos, eliminando la necesidad de múltiples constructores.

Gracias a esta característica, el ejemplo anterior se puede simplificar a:
