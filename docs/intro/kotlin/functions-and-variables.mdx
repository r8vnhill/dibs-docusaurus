...
## 🧠 Ejercicio: Técnicas ninja con valores y mutabilidad
...
- Declara un `val` llamado `baseName` que combine los tres fragmentos con interpolación.
- Declara un `var` llamado `uses`, inicializado en 0.
- Incrementa `uses` y retorna una cadena que incluya `baseName` y la cantidad de usos, como:  
  `"Naruto's Ninjutsu: Kage Bunshin no Jutsu (used 1 time(s))"`

### 🧪 Ejemplo de uso

```kotlin
buildTechniqueName("Naruto", attackName = "Kage Bunshin no Jutsu")
// devuelve: "Naruto's Ninjutsu: Kage Bunshin no Jutsu (used 1 time(s))"
```
...
<Hint hints={[
    ...
    <>
        Puedes usar <code>val</code> y <code>var</code> dentro de una expresión usando <code>run</code>.
    </>,
    <>
        Incrementa <code>uses</code> con <code>++</code> antes de interpolarlo.
    </>
]} />
...
fun buildTechniqueName(
    ...
    val baseName = "$userName's $techniqueType: $attackName"
    var uses = 0
    uses++
    "$baseName (used $uses time(s))"
}
```
</Solution>
</Exercise>
