...
:::note `require` vs `error`

Ambas funciones lanzan una excepción, pero con propósitos distintos:

- `require: (Boolean, () -> Any) -> Unit`: valida argumentos o entradas, lanza `IllegalArgumentException` si la condición es falsa.
- `error: (Any) -> Unit`: lanza incondicionalmente una `IllegalStateException`, útil para estados inválidos del programa.

Ambas permiten expresar errores de forma concisa y clara.
