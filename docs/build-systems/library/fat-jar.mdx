...
<Tabs groupId={"gradle"}>
    <TabItem value={"Kotlin DSL"} label={"Kotlin DSL"}>
        ...
        tasks.register<Jar>("fatJar") {
            ...
            archiveClassifier = "all"   // (1)
            duplicatesStrategy = DuplicatesStrategy.EXCLUDE // (2)
            manifest {  // (3)
                ...
                attributes["Build-Date"] = java.time.LocalDateTime.now().toString()
            }
            from(sourceSets.main.get().output)  // (4)
            dependsOn(configurations.runtimeClasspath)  // (5)
            from({  // (6)
                configurations.runtimeClasspath.get()
                    .filter { file -> file.name.endsWith("jar") }   // (7)
                    .map { file -> zipTree(file) }  // (8)
            })
        }
        ```
    </TabItem>
    <TabItem value={"Groovy DSL"} label={"Groovy DSL"}>
        ...
        tasks.register('fatJar', Jar) {
            ...
            def fatJarConfig = project.extensions.getByType(FatJarExtension)
            ...
            from {  // (6)
                configurations.runtimeClasspath.get()
                    .findAll { file -> file.name.endsWith('jar') }  // (7)
                    .collect { file -> zipTree(file) }  // (8)
            }
        }
        ```
    </TabItem>
...
1. **`archiveClassifier`**: Define el sufijo del archivo JAR. En este caso, `all` indica que es un fat JAR, resultando en un archivo como `lib-1.0.0-all.jar`.
2. **`duplicatesStrategy`**: Establece cómo manejar archivos duplicados. Usamos `EXCLUDE` para evitar conflictos y reducir el tamaño del JAR.
3. **`manifest`**: Establece los atributos del manifiesto JAR, como el título, la versión de la implementación y la fecha de compilación.
4. **`from(sourceSets.main.get().output)`**: Incluye los archivos compilados del proyecto en el JAR.
5. **`dependsOn configurations.runtimeClasspath`**: Asegura que todas las dependencias de tiempo de ejecución se incluyan antes de crear el fat JAR.
6. **`from { ... }`**: Añade las dependencias externas al JAR.
7. **`.filter { file -> file.name.endsWith("jar") }`**: Filtra para incluir solo archivos JAR.
8. **`.map { file -> zipTree(file) }`**: Descomprime los JARs para incluir su contenido en el fat JAR.
...
En la próxima lección, veremos cómo **refinar la tarea de copiado** para tener mayor control sobre qué archivos se copian y dónde se colocan. Aprenderemos a filtrar archivos, excluir ciertos tipos y automatizar aún más el proceso de creación y distribución de archivos en nuestra biblioteca.
