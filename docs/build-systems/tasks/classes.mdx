...
1. **Clase `FibonacciTask`**: Definimos una clase que extiende `DefaultTask`. Lo más común es que las tareas en Gradle hereden de `DefaultTask`. Notamos que la clase es **abstracta**, lo que indica que ciertos parámetros de la
...
tasks.register('processText', TextProcessingTask) {
    inputFile = file('input.txt')
    outputFile = file('output.txt')
    doFirst { println('Processing text...') }
    doLast { println('Processing complete.') }
