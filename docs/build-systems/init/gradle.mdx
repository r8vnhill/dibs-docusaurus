...
```mermaid
graph LR
    A[📁 ./echo-app/] --> A1[📄 .gitattributes]
    A --> A2[📄 .gitignore]
    A --> A3[⚙️ build.gradle.kts]
    A --> A4[🚀 gradlew]
    A --> A5[🚀 gradlew.bat]
    A --> A6[⚙️ settings.gradle.kts]
    A --> A7[🗂️ .gradle]
    A --> A8[📁 gradle]
    A8 --> A81[🧩 libs.versions.toml]
    A8 --> A82[📁 wrapper]
    A82 --> A821[📦 gradle-wrapper.jar]
    A82 --> A822[📝 gradle-wrapper.properties]
```
