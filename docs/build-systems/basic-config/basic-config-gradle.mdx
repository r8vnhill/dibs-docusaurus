...
- **[4]** `app`: Actúa como el módulo de la aplicación principal.
- **[5]** `lib`: Es el módulo donde colocaremos la funcionalidad que queremos reutilizar, como nuestras bibliotecas de software.
...
**Gradle**, como muchos otros sistemas de construcción, automatiza la descarga e integración de las dependencias, ya sea desde repositorios remotos como **Maven Central** o **JCenter**, o desde repositorios locales. Sin embargo, Gradle no es el único sistema que ofrece esta funcionalidad.
Algunos ejemplos de sistemas de construcción que descargan y gestionan dependencias son: **Maven**, **npm**, **SBT** y **Cargo**.

En las próximas lecciones, profundizaremos en cómo agregar y manejar dependencias en Gradle, que es una habilidad
...
En esta lección, exploramos la configuración básica de proyectos en Gradle, con énfasis en:

1. **El archivo `settings.gradle.kts`**: Su rol es definir el nombre del proyecto y estructurar los módulos, lo que resulta esencial para la configuración global del proyecto.
2. **Proyectos multi-módulo**: Vimos cómo dividir un proyecto en varios módulos, lo que permite un desarrollo más organizado, mejor mantenimiento y reutilización del código.
3. **Dependencias**: Aprendimos sobre la gestión de dependencias en Gradle, automatizando la descarga y el manejo de bibliotecas externas necesarias para nuestro proyecto.
4. **Catálogo de versiones**: Introdujimos la idea de centralizar versiones de dependencias en un archivo `.toml`, lo que mejora la consistencia y facilita el mantenimiento.

Estas configuraciones nos preparan para gestionar proyectos más complejos de manera eficiente, mejorando la modularidad y la reutilización de código en bibliotecas de software.
