...
class MaxTest extends AnyPropSpec with ScalaCheckPropertyChecks with Matchers {

  property("max should return the last element of the sorted list") {
    forAll(Gen.listOf(Gen.choose(Int.MinValue, Int.MaxValue))) { list =>
      whenever(list.nonEmpty) {
        max(list) shouldEqual list.sorted.last
      }
    }
  }
