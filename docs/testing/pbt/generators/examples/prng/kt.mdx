---
title: Generadores a partir de un PRNG
---
...
Una de las formas más simples de crear un generador en Kotest es utilizando la función de alto orden `arbitrary`, que aprovecha un **Pseudo-Random Number Generator** manejado internamente por Kotest. Esta función permite generar datos de prueba controlados y reproducibles, útiles para la creación de tests basados en propiedades.
...
<details>
    ...
    <Tabs groupId={"os"}>
        <TabItem value="Windows" label="Windows">
            ...
            ```powershell
            $GeneratorsTestDir = "generators\src\test\kotlin\$Group\generators"
            $GeneratorsMainDir = "generators\src\main\kotlin\$Group\generators"
            $AverageTestDir = "$GeneratorsTestDir\average"
            $AverageMainDir = "$GeneratorsMainDir\average"
            New-Item -Path $AverageTestDir -ItemType Directory -Force
            New-Item -Path $AverageMainDir -ItemType Directory -Force
            $AverageTestDir | Out-File -FilePath "$AverageTestDir\AverageTest.kt"
            $AverageMainDir | Out-File -FilePath "$AverageMainDir\Average.kt"
            "package $Group.generators.average" -replace '\\', '.' | 
                Out-File -FilePath "$AverageTestDir\AverageTest.kt"
            "package $Group.generators.average" -replace '\\', '.' |
                Out-File -FilePath "$AverageMainDir\Average.kt"
            ```
        </TabItem>
        <TabItem value="Windows (corto)" label="Windows (corto)">
            ...
            $AverageTestDir = "$GeneratorsTestDir\average"
            $AverageMainDir = "$GeneratorsMainDir\average"
            $AverageTestDir, $AverageMainDir | % { md $_ }
            "package $Group.generators.average" -replace '\\', '.' | 
                Out-File -FilePath "$AverageTestDir\AverageTest.kt"
            "package $Group.generators.average" -replace '\\', '.' |
                Out-File -FilePath "$AverageMainDir\Average.kt"
        ...
        <TabItem value="Linux/Mac" label="Linux/Mac">
            ...
            ```bash
            GENERATORS_TEST_DIR="generators/src/test/kotlin/$GROUP/generators"
            GENERATORS_MAIN_DIR="generators/src/main/kotlin/$GROUP/generators"
            AVERAGE_TEST_DIR="$GENERATORS_TEST_DIR/average"
            AVERAGE_MAIN_DIR="$GENERATORS_MAIN_DIR/average"
            mkdir -p "$AVERAGE_TEST_DIR" "$AVERAGE_MAIN_DIR"
            echo "package ${GROUP//\//.}.generators.average" > \
                "$AVERAGE_TEST_DIR/AverageTest.kt"
            echo "package ${GROUP//\//.}.generators.average" > \
                "$AVERAGE_MAIN_DIR/Average.kt"
            ```
        </TabItem>
    </Tabs>
</details>
...
<Tabs>
    ...
    <TabItem label={"Código completo"} value={"Código completo"}>
        ```kotlin showLineNumbers title="generators/src/test/kotlin/com/github/username/generators/average/AverageTest.kt"
...
<Exercise>
    ...
    2. Para generar los caracteres aleatorios que componen los valores del diccionario, puedes utilizar la función `nextInt` para obtener números aleatorios dentro del rango de valores Unicode (entre `0x0000` y `0xFFFF`).
    3. Una vez generado el número, conviértelo en un carácter usando la función de extensión `toChar()`.
