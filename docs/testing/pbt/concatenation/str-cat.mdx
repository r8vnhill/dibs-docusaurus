...
<details>
    <summary>Creemos un módulo para los tests...</summary>

    <Tabs groupId={"os"}>
        <TabItem value="Windows" label="Windows">
            ```powershell
            New-Item -Path "str-len\src\test\kotlin\com\github\username\strings\StringConcatenationTest.kt" `
                -ItemType File `
                -Force  # Crea directorios intermedios si no existen
            Set-Content -Path "str-len\build.gradle.kts" -Value "// Intentionally left blank"
            ```
        </TabItem>
        <TabItem value={"Windows (corto)"} label={"Windows (corto)"}>
            ```powershell
            ni "str-len\src\test\kotlin\com\github\username\strings\StringConcatenationTest.kt" `
                -ItemType File `
                -Force
            "// Intentionally left blank" > "str-len\build.gradle.kts"
            ```
        </TabItem>
        <TabItem value="Unix" label="Unix">
            ```bash
            mkdir -p str-len/src/test/kotlin/com/github/username/strings
            touch str-len/src/test/kotlin/com/github/username/strings/StringConcatenationTest.kt
            echo "// Intentionally left blank" > str-len/build.gradle.kts
            ```
        </TabItem>
    </Tabs>

    Recuerda agregar el módulo de pruebas a tu archivo `settings.gradle.kts` para que Gradle pueda detectarlo.
</details>
