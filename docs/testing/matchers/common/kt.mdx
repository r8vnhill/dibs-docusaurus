...
<Explanation>
  ...
  Gracias a que `UserConfig` es una `data class`, Kotlin implementa automáticamente `equals()` para comparar los campos.

  Esta forma de comparar objetos es útil al diseñar bibliotecas, ya que permite testear el **contenido esperado** sin depender de referencias exactas.
</Explanation>
...
Esto asegura que los contenidos no son equivalentes según `equals()`.

<Explanation>
  - `a shouldBe b` → verifica que `a == b`
  - `a shouldNotBe b` → verifica que `a != b`
...
Este matcher compara identidad referencial (`===`), no contenido lógico.
