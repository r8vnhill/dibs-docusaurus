...
- **[2-9]** `toBeEven`: Define un matcher que verifica si un número es par.
- **[3-8]** `compare`: Esta función compara el valor actual (`actual`) con la condición esperada. Si la condición se cumple (el número es par), establece `pass` en `true`; de lo contrario, en `false`.
- **[6]** `message`: Proporciona un mensaje de error cuando la prueba falla, que se mostrará en el reporte de pruebas.
