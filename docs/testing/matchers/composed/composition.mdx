...
<details>
    ...
    <Tabs groupId={"os"}>
        <TabItem value="Windows" label="Windows">
            ...
            ```powershell
            $MatchersDir = "matchers\src\test\kotlin\$Group\matchers\composed"
            New-Item -Path $MatchersDir -ItemType Directory -Force
            "$MatchersDir\BeStrongPassword.kt", "$MatchersDir\BeStrongPasswordTest.kt" |
                ForEach-Object { 
                    "package $Group.matchers.composed" -replace '\\', '.' | 
                        Out-File -FilePath "$_.kt" 
                }
            ```
        </TabItem>
        <TabItem value="Windows (corto)" label="Windows (corto)">
            ...
            md $MatchersDir
            "$MatchersDir\BeStrongPassword.kt", "$MatchersDir\BeStrongPasswordTest.kt" |
                % { "package $Group.matchers.composed" -replace '\\', '.' > $_ }
            ```
        </TabItem>
        <TabItem value="Linux/Mac" label="Linux/Mac">
            ...
            ```bash
            MATCHERS_DIR="matchers/src/test/kotlin/$GROUP/matchers/composed"
            mkdir -p "$MATCHERS_DIR"
            for FILE in "$MATCHERS_DIR/BeStrongPassword.kt" "$MATCHERS_DIR/BeStrongPasswordTest.kt"; do
                echo "package ${GROUP//\//.}.matchers.composed" > \
                    "$FILE"
            done
            ```
        </TabItem>
