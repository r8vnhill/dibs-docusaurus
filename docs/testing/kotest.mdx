...
<ModuleSetup module='kotest-intro' task='setupKotestModule'>
    ...
    fun createGradleBuild() = baseDir.resolve("build.gradle.kts")
        .takeIf { !it.exists() }?.apply {
            writeText("// Intentionally left blank")
            println("File created: $this")
        } ?: printError("File already exists: ${baseDir.resolve("build.gradle.kts")}")

    fun createMyTestFile() {
        val packageName = "${rootProject.group}.testing.basic"
        val packagePath = packageName.replace(".", "/")
        val testFile = baseDir.resolve("src/test/kotlin/$packagePath/MyTest.kt")
        testFile.parentFile.mkdirs()
        testFile.takeIf { !it.exists() }?.apply {
            writeText("package $packageName\n\n")
            println("File created: $this")
        } ?: printError("File already exists: $testFile")
    }
    ```
</ModuleSetup>
