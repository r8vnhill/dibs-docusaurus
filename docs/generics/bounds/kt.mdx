...
<details>
    ...
    <Tabs groupId={"os"}>
        <TabItem value="Windows" label="Windows">
            ...
            New-Item -Path $RepoMainDir -ItemType Directory -Force
            New-Item -Path $RepoTestDir -ItemType Directory -Force
            "package $Group.repo" -replace '\\', '.' | 
                Out-File -FilePath "$RepoMainDir\Repository.kt"
            "package $Group.repo" -replace '\\', '.' |
                Out-File -FilePath "$RepoMainDir\Entity.kt"
            "package $Group.repo" -replace '\\', '.' |
                Out-File -FilePath "$RepoMainDir\Serializable.kt"
            "package $Group.repo" -replace '\\', '.' |
                Out-File -FilePath "$RepoTestDir\RepositoryTest.kt"
            ```
        </TabItem>
        <TabItem value="Windows (corto)" label="Windows (corto)">
            ```powershell
            ...
            md $RepoTestDir
            "package $Group.repo" -replace '\\', '.' > `
                "$RepoMainDir\Repository.kt"
            "package $Group.repo" -replace '\\', '.' > `
                "$RepoMainDir\Entity.kt"
            "package $Group.repo" -replace '\\', '.' > `
                "$RepoMainDir\Serializable.kt"
            "package $Group.repo" -replace '\\', '.' > `
                "$RepoTestDir\RepositoryTest.kt"
            ```
        </TabItem>
        <TabItem value="Linux/Mac" label="Linux/Mac">
            ```bash
            ...
            echo "package ${GROUP//\//.}.repo" > "$REPO_MAIN_DIR/Repository.kt"
            echo "package ${GROUP//\//.}.repo" > "$REPO_MAIN_DIR/Entity.kt"
            echo "package ${GROUP//\//.}.repo" > "$REPO_MAIN_DIR/Serializable.kt"
            echo "package ${GROUP//\//.}.repo" > "$REPO_TEST_DIR/RepositoryTest.kt"
            ```
        </TabItem>
    </Tabs>
</details>
...
### Beneficios y limitaciones de las cotas superiores
...
<details>
    ...
    <Tabs groupId={"os"}>
        <TabItem value="Windows" label="Windows">
            ...
            New-Item -Path $NotificationsMainDir -ItemType 'directory' -Force
            New-Item -Path $NotificationsTestDir -ItemType 'directory' -Force
            "package $Group.notifications" -replace '\\', '.' | 
                Out-File -FilePath "$NotificationsMainDir\EmailNotificationSystem.kt"
            "package $Group.notifications" -replace '\\', '.' |
                Out-File -FilePath "$NotificationsMainDir\NotificationHandler.kt"
            "package $Group.notifications" -replace '\\', '.' |
                Out-File -FilePath "$NotificationsTestDir\EmailNotificationSystemTest.kt"
            ```
        </TabItem>
        <TabItem value="Windows (corto)" label="Windows (corto)">
            ```powershell
            $NotificationsMainDir = "$SrcMainDir\notifications"
            $NotificationsTestDir = "$SrcTestDir\notifications"
            md $NotificationsMainDir
            md $NotificationsTestDir
            "package $Group.notifications" -replace '\\', '.' > `
                "$NotificationsMainDir\EmailNotificationSystem.kt"
            "package $Group.notifications" -replace '\\', '.' > `
                "$NotificationsMainDir\NotificationHandler.kt"
            "package $Group.notifications" -replace '\\', '.' > `
                "$NotificationsTestDir\EmailNotificationSystemTest.kt"
            ```
        </TabItem>
        <TabItem value="Linux/Mac" label="Linux/Mac">
            ```bash
            ...
            echo "package ${GROUP//\//.}.notifications" > "$NOTIFICATIONS_MAIN_DIR/EmailNotificationSystem.kt"
            echo "package ${GROUP//\//.}.notifications" > "$NOTIFICATIONS_MAIN_DIR/NotificationHandler.kt"
            echo "package ${GROUP//\//.}.notifications" > "$NOTIFICATIONS_TEST_DIR/EmailNotificationSystemTest.kt"
            ```
        </TabItem>
    </Tabs>
</details>
