...
import { LanguageCard, JavaCard, RustCard } from '@site/src/components/cards/LanguageCard'
...
<Tabs>
    <TabItem label={"Especificación"} value={"Especificación"}>
        ```kotlin
    ...
    <TabItem label={"Código esencial"} value={"Código esencial"}>
        ...
        "when calling it with a string" - {
            "should return the same string" {
                identity("Hello") shouldBe "Hello"
            }
        }

        "when calling it with an integer" - {
            "should return the same integer" {
                identity(42) shouldBe 42
            }
        }
        ```
    </TabItem>
