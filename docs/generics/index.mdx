...
:::tip Ventajas del Polimorfismo Paramétrico

1. **Reutilización de Código**: Al no limitar una función o estructura de datos a un tipo específico, se puede reutilizar con diferentes tipos de datos.
2. **Seguridad de Tipos**: El sistema de tipos puede garantizar que el código funcione correctamente con cualquier tipo permitido, evitando errores de tipo en tiempo de compilación.
3. **Generalidad**: Permite escribir funciones y estructuras de datos más generales, lo que hace que el código sea más flexible y adaptable.
...
### Sintaxis de Tipos Genéricos en Kotlin

En Kotlin, podemos usar el polimorfismo paramétrico definiendo tipos genéricos con la sintaxis `T`, `U`, `V`, etc. Aquí `T` es un parámetro de tipo genérico, y puede ser sustituido por cualquier tipo concreto cuando se usa la función o clase.

### Ejemplo Básico: Función de Identidad
