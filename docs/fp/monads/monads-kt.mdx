...
Una mónada se define utilizando un **constructor de tipo** `M` y dos operaciones clave:

- **`pure: (A) -> M<A>`**: También conocida como `unit` o `return` en algunos lenguajes, esta función toma un valor de tipo `A` y lo envuelve en una mónada `M<A>`. Su objetivo es crear una mónada a partir de un valor puro, sin introducir efectos secundarios.
- **`flatMap: M<A>.((A) -> M<B>) -> M<B>`**: También llamada `bind` o `chain` (a menudo representada como `>>=`), esta operación toma un valor monádico `M<A>` y una función que convierte ese valor de tipo `A` en una nueva mónada `M<B>`. `flatMap` permite encadenar operaciones monádicas de manera segura, gestionando la propagación de los efectos que puedan ocurrir durante las transformaciones.

> Podemos imaginar una mónada como una caja que encapsula un valor. Con la función `pure`, colocamos un valor dentro de la caja, y con `flatMap`, podemos tomar el valor de la caja, transformarlo, y volver a encapsularlo en otra caja.
...
1. (4) **`pure`**: El método `pure` es una función de fábrica que toma un valor de tipo `T` y lo encapsula dentro de una instancia de `Monad`. Este método es responsable de envolver un valor en la mónada, creando el contexto necesario.
2. (2) **`flatMap`**:
