...
import { LanguageCard, RustCard } from '@site/src/components/cards/LanguageCard'
...
```kotlin showLineNumbers
fun calculateArea(radius: Double): Result<Double> = 
    if (radius >= 0) {
        Success(Math.PI * radius * radius)
    } else {
        Failure(IllegalArgumentException("El radio no puede ser negativo"))
    }
```

Ahora utilizamos el functor `Result` para calcular el área y aplicar transformaciones adicionales solo en caso de éxito, sin necesidad de capturar excepciones.

```kotlin showLineNumbers
fun main() {
    val radius = 5.0
    val areaResult = calculateArea(radius)

    val result = areaResult.map { area -> 
        "El área del círculo con radio $radius es $area"
    }
...
Si ejecutamos el código con un radio positivo, obtenemos un `Success` con el área calculada y transformada en un mensaje descriptivo. Si usamos un radio negativo, obtenemos un `Failure` con el mensaje de error.
...
<ProCons>
    ...
    <Cons>
        ...
        - **Curva de aprendizaje**: Aunque el uso de `Result` es beneficioso, desarrolladorxs que provienen de lenguajes que utilizan excepciones tradicionales pueden encontrar una pequeña curva de aprendizaje al acostumbrarse a la manipulación explícita de errores.
