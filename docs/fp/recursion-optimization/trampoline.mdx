...
<details>
    ...
    ```typescript
    type Trampoline<T> = Done<T> | More<T>;
    ...
    interface More<T> {
        type: 'More';
        continuation: () => Trampoline<T>;
    }

    function Done<T>(result: T): Done<T> {
        return { type: 'Done', result };
    }

    function More<T>(continuation: () => Trampoline<T>): More<T> {
    ...
    ```typescript
    function sumFirstNaturals(n: number, acc: number = 0): Trampoline<number> {
        if (n === 0) {
            return Done(acc);
        } else {
    ...
    ```typescript
    const result = runTrampoline(sumFirstNaturals(100000));
    console.log(result);
    ```
    ...
    class Trampoline(ABC, Generic[T]):
        pass

    # Caso Done con tipo genérico T
    @dataclass
    class Done(Trampoline[T]):
        result: T

    # Caso More con tipo genérico T
    @dataclass
    class More(Trampoline[T]):
        continuation: Callable[[], Trampoline[T]]
    ```

    Implementamos la función recursiva usando el trampolín:

    ```python
    def sum_first_naturals(n: int, acc: int = 0) -> Trampoline[int]:
        if n == 0:
            return Done(acc)
        else:
    ...
    En Rust, podemos implementar el trampolín utilizando enumeraciones (``enum``) y clausuras. Aunque Rust soporta optimización de recursión de cola, el trampolín es útil para casos más complejos.
    ...
    fn sum_first_naturals(n: i32, acc: i32) -> Trampoline<i32> {
        if n == 0 {
            Trampoline::Done(acc)
        } else {
            Trampoline::More(Box::new(move || sum_first_naturals(n - 1, acc + n)))
        }
    }

    fn run_trampoline(mut trampoline: Trampoline) -> i32 {
        loop {
            match trampoline {
                Trampoline::Done(result) => return result,
                Trampoline::More(next) => trampoline = next(),
            }
        }
    }
    ...
    // Ejemplo
    fn main() {
        let result = run_trampoline(sum_first_naturals(100000, 0));
        println!("{}", result);  // Imprime 5000050000
    }
    ```

    ### Clojure

    En **Clojure**, se puede utilizar la función `trampoline` incorporada, que convierte una llamada recursiva en iteración, evitando así desbordamientos de pila.

    ```clojure
    (defn sum-first-naturals [n acc]
        (if (zero? n)
            acc
            #(sum-first-naturals (dec n) (+ acc n))))

    (def result (trampoline sum-first-naturals 100000 0))
    (println result)
    ```

    Clojure usa `#()` para definir funciones de continuación de manera diferida, y `trampoline` es invocado para ejecutar las llamadas recursivas.

    ### Ruby

    En **Ruby**, la recursión de cola no es optimizada, por lo que el método del trampolín es útil para evitar el desbordamiento de pila.
    ...
    def sum_first_naturals(n, acc = 0)
        if n == 0
            Done.new(acc)
        else
            More.new { sum_first_naturals(n - 1, acc + n) }
        end
    end

    def run_trampoline(trampoline)
        loop do
            return trampoline.result if trampoline.is_a?(Done)
            trampoline = trampoline.continue
        end
    end
    ...
    Aquí usamos clases `Done` y `More`, y la función `run_trampoline` ejecuta la continuación hasta obtener el resultado final.
</details>
