...
Aunque cualquier tipo puede ser representado como una composición de pares, como $A \to (B \to C)$, una alternativa más directa es el uso de **tuplas**. Una tupla es un tipo producto que contiene un número fijo de valores de diferentes tipos. Cada valor en la tupla puede ser de un tipo distinto, y la cantidad de valores define la "dimensión" de la tupla (e.g., una tupla de tres elementos es una "triple").
