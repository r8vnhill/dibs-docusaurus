...
- **Scala**: Utiliza `sealed classes` para representar conjuntos finitos de tipos. Scala 3 introduce `union types`, lo que permite que un valor pueda ser de uno de varios tipos posibles.
- **F#**: Usa las `discriminated unions` para definir tipos que representan varias opciones de valor.
- **Rust**: Emplea las `enums` para manejar tipos suma, permitiendo que cada variante de la enumeración tenga su propia estructura de datos.
- **TypeScript**: Introduce los `union types` para permitir que una variable pueda ser de varios tipos posibles.
- **Swift**: Usa `enums` que pueden contener datos asociados para representar tipos suma.
