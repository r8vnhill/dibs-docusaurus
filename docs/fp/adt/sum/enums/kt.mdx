...
<details>
    ...
    <BoxedTabs groupId={"os"}>
        <TabItem value="Windows" label="Windows">
            ...
            $SumMainDir = "adt\src\main\kotlin\$Group\sum"
            $EnumMainDir = "$SumMainDir\enums"
            New-Item -Path $EnumMainDir -ItemType Directory -Force
            "package $Group.sum.enums" -replace '\\', '.' | 
                Out-File -FilePath "$EnumMainDir\DeliveryState.kt"
            ```
        </TabItem>
        ...
        <TabItem value="Linux/Mac" label="Linux/Mac">
            ...
            ```bash
            SUM_MAIN_DIR=adt/src/main/kotlin/$GROUP/sum
            ENUM_MAIN_DIR=$SUM_MAIN_DIR/enums
            mkdir -p $ENUM_MAIN_DIR
            echo "package ${GROUP//\//.}.sum.enums" > \
                "$ENUM_MAIN_DIR/DeliveryState.kt"
            ```
        </TabItem>
    </BoxedTabs>
</details>
...
<Explanation>
Aquí, `DeliveryState` es una enumeración que representa los estados posibles de un pedido: `PENDING`, `PAID`, `SHIPPED`, `DELIVERED` y `CANCELLED`. Cada uno de estos valores es un caso específico de un estado de entrega y puede ser utilizado para modelar el ciclo de vida de un pedido. Al utilizar esta enumeración, se garantiza que solo se utilicen estados válidos y conocidos.
</Explanation>
