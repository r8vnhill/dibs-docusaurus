...
```kotlin
// highlight-next-line
fun factorial(n: Int): Int = if (n == 0) {
    1
} else {
    n * factorial(n - 1)
}
...
Para evitar esto, es necesario **especificar explícitamente el tipo de retorno** de las funciones recursivas. Por ejemplo, en el caso de `factorial`, es mejor declarar el tipo como `Long` para permitir manejar números más grandes:
