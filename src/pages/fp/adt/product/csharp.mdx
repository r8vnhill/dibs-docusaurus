...
### Comparación con C#: Records y Structs
...
Console.WriteLine(newPerson);  // Output: Person { Name = Bob, Age = 30 }
```
...
Las tuplas en **C#** se pueden desestructurar fácilmente en variables individuales utilizando la sintaxis de paréntesis:
...
Esto es similar a la desestructuración de tuplas en **Kotlin**:

```kotlin
val (number, greeting, isActive) = Triple(1, "hello", true)
...
En **C#**, cuando trabajas con **records**, puedes implementar un método `Deconstruct` para permitir la desestructuración de los valores de un record en variables individuales. Por defecto, los **records** en C# ya proporcionan un método `Deconstruct`, por lo que puedes desestructurarlos sin esfuerzo adicional.
...
Esto es equivalente a la desestructuración en **Kotlin** utilizando **data classes**:
